                    ...
                    <tr>
                        ...
                        <td class="actions da-icon-column">
                            <?= $this->Html->link($this->Html->image('icons/color/magnifier.png', ['alt' => 'Ver']),
                                ['action' => 'ver', $materiale->id],
                                ['class' => 'da-tooltip-n', 'title' => 'Ver Material', 'escape' => false]) ?>
                            <?= $this->Html->link($this->Html->image('icons/color/pencil.png', ['alt' =>'Editar']),
                                ['action' => 'editar', $materiale->id],
                                ['class' => 'da-tooltip-n', 'title' => 'Editar Material', 'escape' => false]) ?>
                            <?= $this->Form->postLink($this->Html->image('icons/color/cross.png', ['alt' => 'Eliminar']),
                                ['action' => 'eliminar', $materiale->id],
                                [ 'class' => 'da-tooltip-n', 'title' => 'Eliminar Material', 'confirm' => __('¿Estás seguro de querer eliminar # {0}?', $materiale->id), 'escape' => false]) ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
